...
/// 数字人民币账单解析器
/// 
/// 支持解析数字人民币导出的账单文件（CSV、Excel、PDF格式）
/// 
/// CSV格式示例：
/// ```
/// 交易日期,交易时间,交易类型,对方名称,交易金额,账户余额,交易状态,备注
/// 2024-01-15,12:30:00,支付,美团外卖,-35.00,1234.56,成功,午餐
/// ```
class DigitalRmbParser extends BillParser {
  @override
  BillSource get source => BillSource.digitalRmb;
    ...
    try {
      ...
      for (int i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          print('数字人民币CSV第${i + 1}行解析失败: $e');
        }
      }

      return transactions;
    } catch (e) {
      throw ImportException('数字人民币CSV解析失败: ${e.toString()}');
    }
  }
  ...
  Transaction? _parseCsvRow(List<dynamic> row, String accountId, int rowIndex) {
    ...
    final transactionTypeStr = row.length > 2 ? row[2].toString().trim() : '';
    final counterparty = row.length > 3 ? row[3].toString().trim() : '';
    final amountStr = row.length > 4 ? row[4].toString().trim() : '';
    final status = row.length > 6 ? row[6].toString().trim() : '';
    final remark = row.length > 7 ? row[7].toString().trim() : '';
    ...
    return Transaction(
      ...
      amount: amount.abs(),
      merchantName: counterparty.isNotEmpty ? counterparty : '数字人民币交易',
      description: remark.isNotEmpty ? remark : null,
      transactionDate: transactionDate,
      source: source.name,
      tags: [transactionTypeStr].where((t) => t.isNotEmpty).toList(),
  ...
  Future<List<Transaction>> parseExcel(Uint8List fileBytes, String accountId) async {
    try {
      ...
      throw ImportException('数字人民币Excel解析失败: ${e.toString()}');
    }
  }
  ...
  Transaction? _parseExcelRow(List<Data?> row, String accountId, int rowIndex) {
    ...
    final transactionTypeStr = _getCellValue(row, 2);
    final counterparty = _getCellValue(row, 3);
    final amountStr = _getCellValue(row, 4);
    final status = _getCellValue(row, 6);
    final remark = _getCellValue(row, 7);
  ...
  Future<List<Transaction>> parsePdf(Uint8List fileBytes, String accountId) async {
    try {
      ...
      throw ImportException('数字人民币PDF解析失败: ${e.toString()}');
    }
  }
